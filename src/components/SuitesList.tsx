...
export default function SuitesList({ onNavigate }: SuitesListProps) {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 space-y-4">
        ...
        <span className="text-xs font-mono text-zinc-500 uppercase tracking-widest">Loading suites...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <SectionHeader
        title="Eval Suites"
        subtitle="Group test cases by project or system to track regressions across model runs."
        action={
          <button
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 border border-white/5 rounded-lg bg-zinc-900/50 backdrop-blur-sm font-mono text-xs">
        <div className="flex items-center gap-2">
          <span className="text-zinc-500 uppercase text-[10px]">Filter:</span>
      ...
      {error && (
        <div className="bg-rose-950/20 border border-rose-800/35 p-5 text-rose-400 rounded flex gap-3 text-xs font-mono">
          ...
          <div>
            <span className="font-bold uppercase tracking-wider block mb-1">Failed to load suites</span>
            {error}
          </div>
        </div>
      )}
      ...
      {filteredSuites.length === 0 ? (
        <EmptyState
          ...
          description={filterType !== 'all'
            ? `No suites match the "${filterType}" filter.`
            : "Create a suite to group test cases and start running evaluations."}
          action={
            filterType !== 'all' ? (
              <button
              ...
              <div className="p-5 border-b border-white/5 space-y-3">
                <div className="flex items-start justify-between">
                  <span className="px-2 py-0.5 bg-[#bef264]/10 text-[#bef264] border border-[#bef264]/20 rounded text-[10px] font-mono font-semibold uppercase tracking-wider">
                    {systemTypeLabel(suite.systemType)}
                  </span>
              ...
              <div className="p-5 bg-white/[0.02] flex-1 flex flex-col justify-between space-y-5">
                <div className="grid grid-cols-2 gap-4 font-mono text-xs">
                  <div className="border-l-2 border-zinc-800 pl-3">
                    <span className="text-[10px] text-zinc-500 block leading-tight uppercase">Test Cases</span>
                    <span className="text-lg font-bold text-zinc-200 mt-0.5 block">{suite.caseCount}</span>
                  </div>
                  <div className="border-l-2 border-zinc-800 pl-3">
                    <span className="text-[10px] text-zinc-500 block leading-tight uppercase">Runs</span>
                    <span className="text-lg font-bold text-zinc-200 mt-0.5 block">{suite.runCount}</span>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    {suite.lastRunScore !== undefined ? (
                      <span className={`text-sm font-mono font-bold leading-none mt-1 block ${
                        ...
                        {Math.round(suite.lastRunScore)}%
                      </span>
                    ) : (
                      <span className="text-xs font-mono text-zinc-500 leading-none mt-1 block">No runs yet</span>
                    )}
                  </div>

                  <button
                    onClick={() => onNavigate(`/suites/${suite.id}`)}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-900 hover:bg-zinc-850 hover:border-zinc-500 border border-zinc-800 text-xs font-mono text-zinc-300 transition-colors cursor-pointer"
                  >
